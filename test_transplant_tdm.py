import pytest
from transplant_tdm import TroughTargetWindowAuditorAgent, CYP3A5GenotypeModifierAgent, DoseTitrationCalculatorAgent, TransplantTDMCoordinator, main


def test_sub_agents():
    a1 = TroughTargetWindowAuditorAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = CYP3A5GenotypeModifierAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = DoseTitrationCalculatorAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = TransplantTDMCoordinator()
...
def test_domain_registry():
    from transplant_tdm import DomainKnowledgeRegistry
